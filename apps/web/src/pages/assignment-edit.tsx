...
export function AssignmentEditPage() {
	...
	return (
		<div className="space-y-6">
			<PageTitle title={t("pages.assignmentEdit.pageTitle", { title: assignment.title })} />
			...
			<form onSubmit={handleSubmit} className="space-y-6">
				...
				<Card>
					<CardHeader>
						<CardTitle className="text-base">{t("pages.assignmentEdit.submissionRecordActionTitle")}</CardTitle>
					</CardHeader>
					<CardContent className="space-y-2 text-sm">
						<label className="flex items-center gap-2">
							<input type="radio" name="submissionRecordAction" value="keep" checked={submissionRecordAction === "keep"} onChange={() => setSubmissionRecordAction("keep")} />
							{t("pages.assignmentEdit.keepRecords")}
						</label>
						<label className="flex items-center gap-2">
							<input type="radio" name="submissionRecordAction" value="delete" checked={submissionRecordAction === "delete"} onChange={() => setSubmissionRecordAction("delete")} />
							{t("pages.assignmentEdit.deleteRecords")}
						</label>
						<p className="text-xs text-muted-foreground">{t("pages.assignmentEdit.deleteHelp")}</p>
					</CardContent>
				</Card>
				...
				<div className="flex justify-end gap-3">
					...
					<Button type="submit" disabled={updateMutation.isPending}>
						<Save />
						{updateMutation.isPending ? t("pages.assignmentEdit.updating") : t("pages.assignmentEdit.submit")}
					</Button>
				</div>
			</form>
		</div>
